...
fn intcode_op(pc:usize, data:&mut[i32], op:fn(i32, i32) -> i32) {
    let v0 = data[data[pc + 1] as usize];
    let v1 = data[data[pc + 2] as usize];
    let out = data[pc + 3] as usize;
    data[out] = op(v0, v1);
}

fn intcode_cycle(pc:usize, data:&mut[i32]) -> Option<usize> {
    match data[pc] {
        1 => {
            intcode_op(pc, data, |v0, v1| v0 + v1);
        },
        2 => {
            intcode_op(pc, data, |v0, v1| v0 * v1);
        },
...
fn run_intcode_prg(noun:i32, verb:i32) -> i32 {
    ...
    data[2] = verb;
    let mut pc = 0;
    while let Some(next) = intcode_cycle(pc, &mut data) {
        pc = next;
    }
    data[0]
}
